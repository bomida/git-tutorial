...
### 브랜치 주고받기
  ...
  - 브랜치를 새로 만들어서 만든 브랜치에 바로 체크아웃을 하고싶다면 `git checkout -b 브랜치명`을 입력해주세요.
  - `git branch`를 입력하면 로컬에 있는 브랜치만 확인가능합니다. 하지만, `git branch -a`를 입력하면 로컬과 원격에 있는 브랜치를 볼 수 있습니다.
  <br> 만약 보이지 않는다면 github의 현 상태를 파악하지 않았다는 뜻입니다. 그렇기 때문에 `git fetch`를 실행해서 원격의 소식을 알아봐야합니다.
  - 새 브랜치를 가지고 오기위해서는 `git checkout -b 브랜치명 origin/브랜치명`을 입력해 줍니다.
